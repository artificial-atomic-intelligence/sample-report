...
def load_data(nrows):
    ...
    df_feat = featurizer.featurize_dataframe(df=df, col_id='pmg_composition', ignore_errors=True, pbar=False).drop(columns=['pmg_composition'])
    df_feat['is_metal'] = df_feat['is_metal'].apply(lambda x: 1 if x else 0)
    df_feat['Metal vs Nonmetal'] = df_feat['is_metal'].apply(lambda x: 'Metal' if x else 'Nonmetal')
    return df_feat
...
st.write("This is an example interactive report generated by AtomicAI. "
...
if st.checkbox('Show data'):
    ...
    st.write('There are %d samples and %d descriptors.' % (data.shape[0], 
        data.shape[1]))
...
for idx, col in enumerate(top_3):
    box_fig = px.violin(data, x='Metal vs Nonmetal', y=col, box=True,
...
table = pd.pivot_table(data, index='Metal vs Nonmetal', values=top_3,
